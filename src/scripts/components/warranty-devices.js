...
// Custom element for device list
class WarrantyDevices extends HTMLElement {
  ...
  _renderDevices = async (e) => {
    ...
    try {
      ...
      container.innerHTML = devices.map(this._deviceCardHTML).join('');
    } catch (err) {
      console.error('Failed to load devices:', err);
      container.innerHTML = `<p class="text-red">Failed to load devices: ${err.message}</p>`;
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
  // Unit family/model dropdown logic
  var familySelect = document.getElementById('unit-family');
  var modelSelect = document.getElementById('unit-model');
  ...
  // Warranty device form submission logic
  var form = document.getElementById('warranty-device-form');
  if (form) {
